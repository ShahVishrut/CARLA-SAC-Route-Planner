...
from route_planner_misc import distance_vehicle, is_within_distance_ahead, compute_magnitude_angle, passed_waypoint
...
class RoutePlanner():
  def __init__(self, vehicle, queue_size):
    ...
    self._waypoints_queue = deque(maxlen=queue_size)
    ...
    self._compute_next_waypoints(k=queue_size)
  ...
  def _compute_next_waypoints(self, k=1):
    ...
    for _ in range(k):
      ...
      if len(next_waypoints) == 1:
        ...
        if self.last_turn_time <= 3:
          road_option = self._waypoints_queue[-1][1]
      else:
  ...
  def _get_waypoints(self):
    ...
    if len(self._waypoints_queue) == 0:
      self._waypoints_queue.append( (self._current_waypoint.next(self._sampling_radius)[0], RoadOption.LANEFOLLOW))
    self._compute_next_waypoints(k=15)
   ...
   #     break
    ...
    for i, (waypoint, direction) in enumerate(self._waypoints_queue):
...
def retrieve_options(list_waypoints, current_waypoint):
  ...
  for next_waypoint in list_waypoints:
    ...
    next_next_waypoint = next_next_waypoint.next(3.0)[0]
